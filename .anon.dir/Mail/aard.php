...
# ---------------------------------------------------------------------------------------------------------------------------------------------
   ...
   {
      if(is_assoc_array($o)){$o=knob($o);}; if(!isKnob($o)){fail('expecting object');};
      if(!isPath($o->mesgBody)){fail('expecting `mesgBody` as file path');}; $p=crop($o->mesgBody);
      if(!isee($p)){fail("expecting `$p` as accessible file");}; if(!is_file(path($p))){fail("expecting `$p` as file");};
      if(!isKnob($o->varsUsed)){fail('expecting `varsUsed` as object, even if empty');}; $b=trim(import($p,$o->varsUsed)); $h=stub($b,"\n");
      if($h){$h=stub($h[0],'# ');}; if($h){$h=trim($h[2]);}; if(!$h){fail("expecting `$p` as markdown-formatted text-file with a heading");};
      if(!isMail($o->destAddy)){fail('expecting `destAddy` as email address');}; requires::path('/Proc/libs/marked/Parsedown.php');
      $x=(new \Parsedown()); $x->setBreaksEnabled(true); $b=$x->text($b); $b=import('/Proc/libs/marked/page.htm',['parsed'=>$b]);
      $m=$o->destAddy; $c=$o->fromAddy; if($c){$c=pget("/Mail/link/$c");}else{$c=pget('/Proc/conf/autoMail');};
      if(!isin($c,'mail://')){fail('invalid plug specification .. make sure the `fromAddy` (autoMail) is configured correctly');};
