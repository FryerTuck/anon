...
class PHPMailer
{
    ...
    public function preSend()
    {
        ...
        try {
            ...
            if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
                ...
                fail::email($this->lang('provide_address'));
            }
            ...
            if (!$this->AllowEmpty && empty($this->Body)) {
                ...
                fail::email($this->lang('empty_message'));
            }
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    ...
                    fail::email($this->lang('execute') . $this->Sendmail);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            ...
            fail::email($this->lang('instantiate'));
        }
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect($this->SMTPOptions)) {
            ...
            fail::email($this->lang('smtp_connect_failed'));
        }
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) && !$this->smtp->data($header . $body)) {
            ...
            fail::email($this->lang('data_not_accepted'));
        }
        ...
        if (count($bad_rcpt) > 0) {
            ...
            fail::email($this->lang('recipients_failed') . $errstr);
        }
        ...
        foreach ($hosts as $hostentry) {
            ...
            if (static::ENCRYPTION_STARTTLS === $secure || static::ENCRYPTION_SMTPS === $secure) {
                ...
                if (!$sslext) {
                    ...
                    fail::email($this->lang('extension_missing') . 'openssl');
                }
            }
            $host = $hostinfo[2];
            $port = $this->Port;
                ...
                try {
                    ...
                    if ($tls) {
                        if (!$this->smtp->startTLS()) {
                            ...
                            fail::email($this->lang('connect_host'));
                        }
                    ...
                    if ($this->SMTPAuth && !$this->smtp->authenticate(
                        ...
                        fail::email($this->lang('authenticate'));
                    }
        ...
        if ($this->isError()) {
            ...
            try {
                ...
                if ($sign) {
                    ...
                    fail::email($this->lang('signing') . openssl_error_string());
                }
            } catch (Exception $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment(
        ...
        try {
            if (!static::isPermittedPath($path) || !@is_file($path) || !is_readable($path)) {
                ...
                fail::email($this->lang('file_access') . $path);
            }
            ...
            if (!$this->validateEncoding($encoding)) {
                ...
                fail::email($this->lang('encoding') . $encoding);
            }
    ...
    protected function encodeFile($path, $encoding = self::ENCODING_BASE64)
    {
        try {
            if (!static::isPermittedPath($path) || !file_exists($path) || !is_readable($path)) {
                ...
                fail::email($this->lang('file_open') . $path);
            }
    ...
    public function addCustomHeader($name, $value = null)
    {
        ...
        if (empty($name) || strpbrk($name . $value, "\r\n") !== false) {
            if ($this->exceptions) {
                ...
                fail::email('Invalid header name or value');
            }
