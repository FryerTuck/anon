...
# ---------------------------------------------------------------------------------------------------------------------------------------------
   class dbug
   {
      ...
      {
         $o->mesg=self::wash($o->mesg); $o->file=self::wash($o->file); $x=fext(NAVIPATH); $m=$o->mesg; // scrub sys-paths
         $s=[]; foreach($o->stak as $i){$s[]="$i->func $i->file $i->line";}; $o->stak=$s; $n=$o->name; $f=$o->file; $l=$o->line; $t=tval($o);

         if(facing('API')){ekko($t); exit;};  if(facing('DPR')&&($x=='js')){ekko("fail($t)");}; // API & js-dpr
         if(facing('DPR')){$m=str_replace(["\n",'"'],['',"`"],$m); $m=crop($m,60); harakiri("$n - $m - $f - $l"); exit;}; // any other file
