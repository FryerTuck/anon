...
# ---------------------------------------------------------------------------------------------------------------------------------------------
   function skey()
   {
      ...
      if($n){kuki($n,null); if(isset($_GET['test'])){die("503 test 3");}; header($z); exit;}; // bad session key
      ...
      $u=pget("/Proc/keys/$r"); if(!$u){kuki($r,null); header($z);}; // invalid session key .. YOU HAVE DIED
